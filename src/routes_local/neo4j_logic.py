...
def valueset_get(neo4jInstance, parent_sab: str, parent_code: str, child_sabs: List[str]) -> List[SabCodeTerm]:
    ...
    with neo4jInstance.driver.session() as session:
...
def get_organ_types(neo4jInstance, sab):
    result = []
    query = \
        "CALL " \
        "{ " \
    ...
    with neo4jInstance.driver.session() as session:
        ...
        for record in recds:
            item = {
                    'sab': record.get('OrganSAB'),
                    'code': record.get('OrganCode'),
                    'term': record.get('OrganName'),
                    'rui_code': record.get('OrganTwoCharacterCode'),
                    'organ_uberon': record.get('OrganUBERON')
            }
            result.append(item)
    return result


def dataset_get(neo4jInstance, application_context: str, data_type: str = '', description: str = '', alt_name: str = '',
            ...
            if includerecord:
                try:
                    dataset = {
                        ...
                        'contains_pii': record.get('contains_pii'),
                        'data_type': record.get('data_type'),
                        'dataset_provider': record.get('dataset_provider'),
                        'description': record.get('description'),
                        'primary': record.get('primary'),
                        'vis_only': record.get('vis_only'),
                        'vitessce_hints': record.get('vitessce_hints')
                    }
...
def __subquery_dataset_synonym_property(sab: str, cuialias: str, returnalias: str,
